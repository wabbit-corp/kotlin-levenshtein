 ...
 *
 * dp[i][j] = if (isEqual(i-1,j-1)) then dp[i-1][j-1]+1 else 0 Keep track of maximum dp[i][j].
 */
...
fun longestCommonSubstring(lhs: CharSequence, rhs: CharSequence, ignoreCase: Boolean = false): Int =
    longestCommonSubstringCommon(
        lhs.length,
        rhs.length,
        isEqual = { i, j ->
            if (ignoreCase) {
...
/**
 * Data class to store the result of the Longest Common Substring, including the substring itself
 * and the start indices in LHS and RHS.
 */
...
/**
 * Returns the length of the longest common substring plus the actual substring text. Also provides
 * the starting index in each string (lhs and rhs).
 *
...
fun longestCommonSubstringWithPath(
    ...
    val m = rhs.length
    if (n == 0 || m == 0) {
        return LongestCommonSubstringResult(length = 0, substring = "", lhsIndex = 0, rhsIndex = 0)
    }
